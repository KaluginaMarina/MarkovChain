...
то увеличивается вероятность перехода из состояния первого слова в паре в состояние второго. Если в автомате нет хотя бы одного слова, то сначала сохдаются недостающие вершины
в граф, а потом добавляется отношение между ними.

### Генерация текста

Генерация текста использует автомат, основанный на тексте.
